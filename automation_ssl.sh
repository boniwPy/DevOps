#!/bin/bash
#=============================================================#
# Name:         Script Automation install ssl                 #
# Description:  Install autossl dengan gampang                #
...
#                   USAGE:
#      chmod +x ./automation_ssl.sh
#      sudo ./automation_ssl.sh
...
checkRoot() {
   if [ $(id -u) -ne 0 ]; then
     printf "Script must be run as root. Try 'sudo ./automation_ssl.sh'\n"
     exit 1
   fi
}
...
setServerName() {
    cmd=(dialog --backtitle "BoniW - Automation Instaling SSL" \
...
installCertificateNginx() {
  dialog --backtitle "BoniW - Automation Instaling SSL" \
...
while true; do
    cmd=(dialog --backtitle "BoniW - Automation Instaling SSL" \
                --title "Automation Instaling SSL" \
